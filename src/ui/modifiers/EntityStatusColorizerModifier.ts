...
export class EntityStatusColorizerModifier implements IModifier {
    private readonly _allowedPages: string[] = [
        'attractions.php',
        'restaurants.php',
        'entrance.php',
    ];
    ...
    private _colorizeAttractionStatuses(): void {
        const elements = document.querySelectorAll<HTMLElement>(
            'article[class="owned-attraction-card"]',
        );
    ...
    private _colorizeRestaurantStatuses(): void {
        const elements = document.querySelectorAll<HTMLElement>(
            'article[class="owned-restaurant-card"]',
        );
        this._logger.debug(`Found ${elements.length} restaurant cards to process.`);

        elements.forEach((element) => {
            const status = element
                .querySelector<HTMLElement>('span.owned-restaurant-card__status')
                ?.textContent?.toLowerCase();

            if (status) {
                if (status.includes('ouvert')) element.style.border = '2px solid green';
                else if (status.includes('fermé')) element.style.border = '2px solid red';
                else if (status.includes('en travaux')) element.style.border = '2px dashed orange';
            }
        });
    }
    ...
    private _colorizeEntranceStatuses(): void {
        ...
        elements.forEach((element) => {
            const status = element
                .querySelector<HTMLElement>('span.booth-row__status')
                ?.textContent?.toLowerCase();

            if (status) {
                if (status.includes('non assigné')) element.style.border = '2px solid red';
