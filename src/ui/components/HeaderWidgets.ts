    ...
    private _createChatButton(): HTMLButtonElement {
        return createElement('button', {
            ...
            onclick: () => {
                ...
                if (chatButton) {
                    chatButton.textContent = this._isChatOpen ? STRINGS.CLOSE_CHAT_BUTTON : STRINGS.OPEN_CHAT_BUTTON;
                }

                document.dispatchEvent(new CustomEvent(EVENTS.CHAT_TOGGLED, {
                    detail: { isOpen: this._isChatOpen }
                }));

                document.body.classList.toggle(SELECTORS.CHAT_OPENED, this._isChatOpen);

                this._logger.debug(`Chat toggled. New state: ${this._isChatOpen ? 'OPEN' : 'CLOSED'}`);
            }
        }, [
